        ...
        <div className="max-w-[1280px] mx-auto px-xl flex flex-wrap items-center gap-md">
          ...
          {CATEGORIES.map(cat => (
            <motion.button
              ...
              className={`px-lg py-sm rounded-full font-semibold text-body-md ${
                active === cat
            ...
            {visible.map(post => (
              <StaggerItem key={post.id}>
                <motion.div whileHover={{ y: -6, boxShadow: '0 24px 50px rgba(65,105,0,0.10)' }} transition={{ duration: 0.25 }}>
                  <Link
                    to={`/blog/${post.id}`}
                    className="group bg-white rounded-lg border border-border-subtle overflow-hidden card-shadow block"
                  >
        ...
        <div className="max-w-[700px] mx-auto px-xl">
          <AnimatedSection>
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.04, backgroundColor: '#f8fef0' }}
              whileTap={{ scale: 0.97 }}
